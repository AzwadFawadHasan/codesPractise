...
class Node {
	public:
			int  key ;int data; Node * next ;// making variables 
			...
			Node (int k , int d )// making a parametrized constructor 
			{
				key = k ;
				 data = d;
				 next = NULL;// when ever you create a node for the first time 
				 //you have to make sure that the next pointer is null
			}
...
int main ()
{
	...
	// if we use just int *temp = new node ();it will store the value in the stack so it will get removed evenuallly  
	
	node * new_node = new node ();// creates a pointer in stack and the node in heap
}
